  ...
  return (
    <>
      <div className='overflow-hidden'>
        <HeaderOne />
        <div className="style-halfmap-list">
          <div className="content-main flex h-full max-xl:flex-col-reverse">
            <div className="left xl:w-7/12 w-full lg:pt-10 md:pt-6 lg:px-10 md:px-6 px-4 max-md:py-8 relative">
              <div className="heading flex items-center justify-between gap-6 flex-wrap">
                <div className="left flex items-center sm:gap-5 gap-3 max-lg:flex-wrap">
              ...
              <div className="list-tent lg:mt-10 md:mt-8 mt-6 flex flex-col gap-[30px]">
                {currentTents.map((item) => (
                  item.id === 'no-data' ? (
                    <div key={item.id} className="no-data-product">No tents match the selected criteria.</div>
                  ) : (
                    <TentItem key={item.id} data={item} type='list-halfmap' />
                  )
                ))}
              </div>

              {pageCount > 1 && (
                <div className="list-pagination flex items-center justify-center md:w-7/12 max-md:mt-6">
                  <HandlePagination pageCount={pageCount} onPageChange={handlePageChange} />
                </div>
              )}
            </div>
            <div className="right xl:w-5/12 w-full max-xl:h-[420px] max-lg:h-[380px]">
              <MapComponent />
            </div>
          </div>
        </div>
