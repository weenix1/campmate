...
const CheckoutPage = ({ amount,id }: { amount: number; id:string }) => {
    ...
    const fetchClientSecret = async () => {
      try {
        const response = await fetch("/api/create-payment-intent", {
          ...
          body: JSON.stringify({ amount: convertToSubcurrency(amount) }), // Convert amount to subunits (cents for example)
        });
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const { error } = await stripe.confirmPayment({
      ...
      clientSecret,
      confirmParams: {
        return_url: `http://localhost:3000/payment-success?amount=${amount}&id=${id}`,
      },
    });
